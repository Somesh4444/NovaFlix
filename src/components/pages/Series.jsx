...
import { Star, Play, ChevronRight, Home, Tv, SlidersHorizontal, Grid, Plus } from "lucide-react";
...
export default function Series() {
  ...
  // SORTING ENGINE UPDATED FOR TV SERIES CONTEXT
  const sortedShows = useMemo(() => {
    const showsCopy = [...shows];
    switch (sortBy) {
      case "rating":
        return showsCopy.sort((a, b) => (b.vote_average || 0) - (a.vote_average || 0));
      case "date":
        return showsCopy.sort((a, b) => new Date(b.first_air_date || 0) - new Date(a.first_air_date || 0));
      case "popularity":
        return showsCopy.sort((a, b) => (b.popularity || 0) - (a.popularity || 0));
      default:
        return shows; 
    }
  }, [shows, sortBy]);
  ...
  useEffect(() => {
    ...
    const fetchSeries = async () => {
      try {
        ...
        // Targeted TMDB's TV Trending Endpoint directly
        ...
        if (isMounted) {
          setShows((prevShows) => {
            ...
            // Duplicate safety filtering
            const uniqueIncoming = incomingResults.filter(
              (newShow) => !prevShows.some((oldShow) => oldShow.id === newShow.id)
            );
            return [...prevShows, ...uniqueIncoming];
          });
        }
      } catch (err) {
        console.error("Failed to fetch series database entries:", err);
        if (isMounted) setError("Failed to load trending series. Please try again later.");
      } finally {
        if (isMounted) {
          setLoading(false);
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center text-white font-medium">
        <div className="flex flex-col items-center gap-3">
          ...
          <span>Loading TV Series Catalog...</span>
        </div>
      </div>
    );
  }
    ...
    <section className="min-h-screen bg-[#020617] text-gray-100 py-12 select-none">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 border-b border-white/5 pb-6">
          ...
          {/* DYNAMIC SHOWS COUNTER & DROPDOWN FILTER */}
          <div className="flex items-center gap-3 self-start sm:self-auto text-xs font-bold text-gray-400">
            <div className="flex items-center gap-1.5 bg-white/5 border border-white/5 px-3 py-2 rounded-xl">
              ...
              <span>{shows.length} Series Indexed</span>
            </div>
        ...
        <div className="mb-6">
          <h2 className="text-2xl md:text-3xl font-black text-white tracking-wide capitalize">
            Your Ultimate TV Series Library
          </h2>
        </div>
              ...
              >
                ...
                <div className="relative aspect-2/3 w-full rounded-xl overflow-hidden bg-slate-900">
                  ...
                  <div className="absolute top-2 left-2 bg-cyan-500/10 backdrop-blur-md border border-cyan-400/30 text-cyan-400 text-[9px] font-black tracking-wider px-2 py-0.5 rounded-md uppercase z-10">
                    {show.vote_average >= 7.5 ? "Blockbuster" : "Trending"}
                  </div>
                ...
                <div className="mt-2.5 px-1 flex flex-col gap-0.5">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-gray-500 font-semibold mt-0.5">
                    <span className="truncate pr-1">Series</span>
        ...
        {/* INFINITE PAGINATION TRIGGER CONTROL */}
        <div className="flex justify-center mt-10">
          ...
          >
            ...
            <span>{loadingMore ? "Loading..." : "Load More Shows"}</span>
          </button>
        </div>
